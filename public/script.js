...
window.onload = () => {
    ...
    socket.on('nextTurn', (data) => {
        document.getElementById('turnCounter').textContent = parseInt(document.getElementById('turnCounter').textContent) + 1;
        handleStatusEffects();
    });
    ...
    socket.on('applyStatus', (data) => {
        ...
        console.log(`Estado recibido para ${username}: ${status}, Duración: ${duration}`);
...
function applyStatus(status) {
    ...
    if (status === 'luck') {
        if (myStatuses[status] && myStatuses[status] > 0) {
            ...
            console.log(`Reiniciando duración para mi estado: ${status}. Duración reiniciada: ${myStatuses[status]}`);
        } else {
            myStatuses[status] = duration; // Asignar la duración inicial
            console.log(`Asignando duración inicial para mi estado: ${status}. Duración inicial: ${myStatuses[status]}`);
        }
        updateMyStatusList();
        socket.emit('applyStatus', { room, username, status, duration: myStatuses[status] });
    } else {
        if (opponentStatuses[status] && opponentStatuses[status] > 0) {
            ...
            console.log(`Reiniciando duración para el estado del oponente: ${status}. Duración reiniciada: ${opponentStatuses[status]}`);
        } else {
            opponentStatuses[status] = duration; // Asignar la duración inicial
            console.log(`Asignando duración inicial para el estado del oponente: ${status}. Duración inicial: ${opponentStatuses[status]}`);
        }
        updateOpponentStatusList();
        socket.emit('applyStatus', { room, username: opponent.username, status, duration: opponentStatuses[status] });
    }
}
...
function updateLife(change, target) {
    if (target === 'my') {
        ...
        socket.emit('updateStats', { room, username, life: currentLife, defense: parseInt(document.getElementById('myDefense').textContent) });
        ...
        socket.emit('updateStats', { room, username: opponent.username, life: currentLife, defense: parseInt(document.getElementById('opponentDefense').textContent) });
    }
}
...
function updateDefense(change, target) {
    if (target === 'my') {
        ...
        socket.emit('updateStats', { room, username, life: parseInt(document.getElementById('myLife').textContent), defense: currentDefense });
    } else if (target === 'opponent') {
        ...
        socket.emit('updateStats', { room, username: opponent.username, life: parseInt(document.getElementById('opponentLife').textContent), defense: currentDefense });
    }
}

function handleStatusEffects() {
    console.log('--- Comenzando manejo de efectos de estado en un nuevo turno ---');

    // Manejar los efectos en el jugador
    for (let status in myStatuses) {
        if (myStatuses[status] > 0) {
            console.log(`Estado actual para mi: ${status}, Duración restante: ${myStatuses[status]}`);
            if (status === 'bleed') {
            ...
            if (myStatuses[status] === 0) {
                ...
                console.log(`Estado ${status} en mi terminó y ha sido eliminado.`);
            }
        }
    }
    ...
    for (let status in opponentStatuses) {
        if (opponentStatuses[status] > 0) {
            console.log(`Estado actual para el oponente: ${status}, Duración restante: ${opponentStatuses[status]}`);
            if (status === 'bleed') {
            ...
            if (opponentStatuses[status] === 0) {
                ...
                console.log(`Estado ${status} en el oponente terminó y ha sido eliminado.`);
            }
        }
    }
...
function updateMyStatusList() {
    ...
    for (let status in myStatuses) {
        if (myStatuses[status] > 0) {
            ...
            statusItem.appendChild(document.createTextNode(` : ${myStatuses[status]} turnos`));
...
function updateOpponentStatusList() {
    ...
    for (let status in opponentStatuses) {
        if (opponentStatuses[status] > 0) {
            ...
            img.style.width = '20px';
            statusItem.appendChild(img);
            statusItem.appendChild(document.createTextNode(` : ${opponentStatuses[status]} turnos`));
            statusList.appendChild(statusItem);
        }
    }
}
...
function applyFireEffect(target) {
    if (target === 'my') {
        let currentDefense = parseInt(document.getElementById('myDefense').textContent);
        if (currentDefense > 0) {
            updateDefense(-1, 'my');
        } else {
            updateLife(-1, 'my');
        ...
        let currentDefense = parseInt(document.getElementById('opponentDefense').textContent);
        if (currentDefense > 0) {
